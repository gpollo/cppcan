...
#include "can/database/dbc/node.hpp"
...
class database : public can::database::database, public object {
   public:
    ...
    database(const ast::database& database, std::map<unsigned int, message::ptr> messages_by_identifier,
    ...
    [[nodiscard]] std::vector<message::const_ptr> get_messages_dbc() const;
    [[nodiscard]] message::const_ptr get_message_dbc(unsigned int identifier) const;
    [[nodiscard]] message::const_ptr get_message_dbc(const std::string& name) const;
    [[nodiscard]] const std::map<std::string, node::ptr>& get_nodes_dbc() const;
    /* TODO: add description fields */

   private:
    std::map<unsigned int, message::ptr> messages_by_identifier_;
    std::map<std::string, message::ptr> messages_by_name_;
