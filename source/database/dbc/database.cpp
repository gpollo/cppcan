...
database::ptr database::from_ast(const ast::database& database) {
    ...
    std::map<unsigned int, message::ptr> messages_by_identifier;
    std::map<std::string, message::ptr> messages_by_name;
    for (const auto& [identifier, message] : database.get_messages()) {
        auto message_ptr = message::from_ast(database, message);
...
database::database(const ast::database& database, std::map<unsigned int, message::ptr> messages_by_identifier,
                   std::map<std::string, message::ptr> messages_by_name, std::map<std::string, node::ptr> nodes_dbc,
...
std::vector<message::const_ptr> database::get_messages_dbc() const {
    std::vector<message::const_ptr> messages;
    for (auto [identifier, message] : messages_by_identifier_) {
        messages.push_back(message);
    }
...
message::const_ptr database::get_message_dbc(unsigned int identifier) const {
...
message::const_ptr database::get_message_dbc(const std::string& name) const {
